...
####################
        ...
        pseudo_label=torch.ones(predicted1.shape,dtype=int)*-1
        predicted1=predicted1.cuda()
        predicted2=predicted2.cuda()
        ...
        m=nn.Softmax(dim=1)
        ensemble_prob=m(ensemble_prediction)
        ensemble_prob_2=[]
        for ensemble_prob_tensor,ensemble_pred_idx in zip(ensemble_prob,ensemble_predicted):
            ensemble_prob_tensor=torch.cat([ensemble_prob_tensor[:ensemble_pred_idx],ensemble_prob_tensor[ensemble_pred_idx+1:]],dim=0)
            _,prob2=ensemble_prob_tensor.max(0)

            ensemble_prob_2.append(ensemble_prob_tensor[prob2])
        mark=[]

        i=0

        for ensemble_prob,ensemble_pred_idx in zip(ensemble_prob,ensemble_predicted):
            if ensemble_prob[ensemble_pred_idx]-ensemble_prob_2[i]<0.1:
                mark.append(-1)
            else:
                mark.append(0)
            i=i+1
...
if __name__ == "__main__":
    ...
    optimizer1_1 = torch.optim.Adam(model1.parameters(), lr=0.0001,betas=(0.9,0.999)) #Your optimizer here
    optimizer2_1 = torch.optim.Adam(model2.parameters(), lr=0.0001,betas=(0.9,0.999)) #Your optimizer here
